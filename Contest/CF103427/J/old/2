...
constexpr int MAXNODE = 20705;
constexpr int MAXEDGE = 50005;
constexpr int INF = 0x3f3f3f3f;
constexpr int dx[5] = {0, 0, 0, 1, -1};
constexpr int dy[5] = {0, 1, -1, 0, 0};
constexpr string s = "SRLDU";
int N, S, T;
struct Edge {
    int v, nxt, w;
}e[MAXEDGE];
...
int dep[MAXNODE], que[MAXNODE], hd, tl, max_flow, ans;
bool bfs() {
    for(int i = 0; i < N; i++) {
        dep[i] = 0;
        cur[i] = head[i];
...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        ...
        cout << sx[i]+1 << ' ' << sy[i]+1 << ' ' << ex[outnumber]+1 << ' ' << ey[outnumber] << '\n';
    }
    return 0;
}
