...
[P4069 游戏](https://www.luogu.com.cn/problem/P4069)

## 题目大意

给一棵树，点有点权。维护以下两种操作：

1. 给出参数 $s,t$ ，使路径 $(s,t)$ 上的每个点 $u$ 的点权对 $a\times dis(s,u)+b$ 取 $\max$。
2. 给出参数 $s,t$，询问路径 $(s,t)$ 上点权最大值。

## 解题报告

如果是一个序列，我们发现这就是李超线段树的模板（区间修改，区间查询）。那么这题怎么办呢？其实只要套一层树剖。

随意钦定一个根，记 $dis(u)$ 表示到根距离。

取 $s,t$ 的 $lca$ 为 $p$，则可以把操作1. 分成两部分：

1. 在路径 $(s,p)$ 上，有 $y=a\times dis(s,u)+b=a(dis(s)-dis(u))+b=-a\cdot dis(u)+a\cdot dis(s)+b$，可以看成是关于 $dis(u)$ 的一次函数。
2. 在路径 $(p,t)$ 上，有 $y=a\times dis(s,u)+b=a(dis(s)+dis(u)-2dis(p))+b=a\cdot dis(u)+a\cdot dis(s) - 2a\cdot dis(p)+b$ 同样可以看成关于 $dis(u)$ 的一次函数。
