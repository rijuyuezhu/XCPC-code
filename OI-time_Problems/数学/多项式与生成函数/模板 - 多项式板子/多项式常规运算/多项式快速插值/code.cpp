...
Poly Add(Poly f, Poly g) {
	...
	for(int i = 0; i < n; i++) f[i] = pls(f[i], g[i]);
	return f;
}
Poly Inv(Poly f) {
	int n = f.size();
	if(n == 1) {Poly g(1); g[0] = qpow(f[0], P-2); return g;}
	Poly g = f; g.resize((n+1)/2); g = Inv(g);
	int lim = getlim(n+n);
	DFT(f, lim); DFT(g, lim);
	for(int i = 0; i < lim; i++) g[i] = mul(g[i], mns(2, mul(f[i], g[i])));
	IDFT(g, lim); g.resize(n);
	return g;
}
namespace Mevaluate {
	...
	void build(int p, int l, int r, const Poly& a) {
		if(l == r) {
			t[p].resize(2); t[p][0] = 1; t[p][1] = mns(0, a[l]);
			return ;
		}
		int m = (l + r) >> 1;
	...
	void calc(int p, int l, int r, Poly& a) {
		...
		int m = (l + r) >> 1;
		swap(t[ls], t[rs]);
		t[ls] = TMul(t[p], t[ls]); t[rs] = TMul(t[p], t[rs]);
		calc(ls, l, m, a); calc(rs, m+1, r, a);
	}
	Poly solve(Poly f, Poly a) {
		int n = f.size(), m = a.size();
		n = max(n, m); f.resize(n); a.resize(n);
		build(1, 0, n-1, a);
		f.resize(2 * n + 1); t[1] = TMul(f, Inv(t[1]));
		calc(1, 0, n-1, a);
		a.resize(m);
		return a;
	}
...
int n;
Poly X, Y, g[MAXN << 2], f[MAXN << 2], h;
#define ls p << 1
#define rs p << 1 | 1
void solve1(int p, int l, int r, const Poly& X) {
	if(l == r) {
		g[p].resize(2); g[p][0] = mns(0, X[l]); g[p][1] = 1;
		return ;
	}
	int m = (l + r) >> 1;
	solve1(ls, l, m, X); solve1(rs, m+1, r, X);
	g[p] = Mul(g[ls], g[rs]);
}
void solve2(int p, int l, int r, const Poly& Y) {
	if(l == r) {
		f[p].resize(1); f[p][0] = mul(Y[l], qpow(h[l], P-2));
		return ;
	}
	int m = (l + r) >> 1;
	solve2(ls, l, m, Y); solve2(rs, m+1, r, Y);
	f[p] = Add(Mul(f[ls], g[rs]), Mul(f[rs], g[ls]));
}
#undef ls
#undef rs
int main() {
